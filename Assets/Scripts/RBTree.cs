...
internal delegate bool TreeWalkPredicate<T>(RBTree<T>.Node node);
    ...
    [Serializable]
    public class RBTree<T> : ISet<T>, ICollection<T>, ICollection, ISerializable, IDeserializationCallback, IReadOnlyCollection<T>
    {
        ...
        public RBTree(IEnumerable<T> collection) : this(collection, Comparer<T>.Default) { }

        public RBTree(IEnumerable<T> collection, IComparer<T> comparer)
            : this(comparer)
        {
            ...
            RBTree<T> baseSortedSet = collection as RBTree<T>;
            RBTree<T> baseTreeSubSet = collection as TreeSubSet;
            if (baseSortedSet != null && baseTreeSubSet == null && AreComparersEqual(this, baseSortedSet))
            {
                ...
                Stack<Node> theirStack = new Stack<RBTree<T>.Node>(2 * log2(baseSortedSet.Count) + 2);
                Stack<Node> myStack = new Stack<RBTree<T>.Node>(2 * log2(baseSortedSet.Count) + 2);
                Node theirCurrent = baseSortedSet.root;
                Node myCurrent = (theirCurrent != null ? new RBTree<T>.Node(theirCurrent.Item, theirCurrent.IsRed) : null);
                root = myCurrent;
                while (theirCurrent != null)
                {
                    theirStack.Push(theirCurrent);
                    myStack.Push(myCurrent);
                    myCurrent.Left = (theirCurrent.Left != null ? new RBTree<T>.Node(theirCurrent.Left.Item, theirCurrent.Left.IsRed) : null);
                ...
                while (theirStack.Count != 0)
                {
                    ...
                    if (theirRight != null)
                    {
                        myRight = new RBTree<T>.Node(theirRight.Item, theirRight.IsRed);
                    }
                    ...
                    while (theirRight != null)
                    {
                        ...
                        myRight.Left = (theirRight.Left != null ? new RBTree<T>.Node(theirRight.Left.Item, theirRight.Left.IsRed) : null);
...
#if !FEATURE_NETCORE
        protected RBTree(SerializationInfo info, StreamingContext context)
        {
            siInfo = info;
        }
#endif
        ...
        /// <summary>
        /// Remove the T ITEM from this RBTree. Returns true if successfully removed.
        /// </summary>
        ...
        void ICollection.CopyTo(Array array, int index)
        {
            ...
            {
                throw new ArgumentException("Lower bound must be 0");
            }
    ...
    internal virtual int InternalIndexOf(T item)
        ...
        /// <summary>
        /// Used for deep equality of RBTree testing
        /// </summary>
        /// <returns></returns>
        public static IEqualityComparer<RBTree<T>> CreateSetComparer()
        {
        ...
        /// </summary>                
        public static IEqualityComparer<RBTree<T>> CreateSetComparer(IEqualityComparer<T> memberEqualityComparer)
        {
        ...
        /// </summary>        
        internal static bool SortedSetEquals(RBTree<T> set1, RBTree<T> set2, IComparer<T> comparer)
        {
        ...
        private static bool AreComparersEqual(RBTree<T> set1, RBTree<T> set2)
        {
        ...
        public void UnionWith(IEnumerable<T> other)
        {
            ...
            {
                RBTree<T> dummy = new RBTree<T>(s, this.comparer);
            ...
            { //this actually hurts if N is much greater than M the /2 is arbitrary
                ...
                root = RBTree<T>.ConstructRootFromSortedArray(merged, 0, c - 1, null);
                count = c;
                version++;
            }
            else
        ...
        public virtual void IntersectWith(IEnumerable<T> other)
        {
            ...
            if (t != null)
                VersionCheck();
            //only let this happen if i am also a RBTree, not a SubSet
        ...
        public void ExceptWith(IEnumerable<T> other)
        {
            ...
            RBTree<T> asSorted = other as RBTree<T>;
        ...
        private bool IsSubsetOfSortedSetWithSameEC(RBTree<T> asSorted)
        {
            RBTree<T> prunedOther = asSorted.GetViewBetween(this.Min, this.Max);
        ...
        public bool IsSupersetOf(IEnumerable<T> other)
        {
            ...
            if (asSorted != null && AreComparersEqual(this, asSorted))
            {
                if (this.Count < asSorted.Count)
                    return false;
                RBTree<T> pruned = GetViewBetween(asSorted.Min, asSorted.Max);
        ...
        public T Min
        {
            ...
            {
                T ret = default(T);
                InOrderTreeWalk(delegate (RBTree<T>.Node n) { ret = n.Item; return false; });
                return ret;
            }
        }
        ...
        public T Max
        {
            ...
            {
                T ret = default(T);
                InOrderTreeWalk(delegate (RBTree<T>.Node n) { ret = n.Item; return false; }, true);
                return ret;
            }
        }
        ...
        public virtual RBTree<T> GetViewBetween(T lowerValue, T upperValue)
        {
...
#if !FEATURE_NETCORE
        [Serializable]
        internal sealed class TreeSubSet : RBTree<T>, ISerializable, IDeserializationCallback
        {
#else
        internal sealed class TreeSubSet : RBTree<T> {
#endif
            RBTree<T> underlying;
            T min, max;
            ...
            public TreeSubSet(RBTree<T> Underlying, T Min, T Max, bool lowerBoundActive, bool upperBoundActive)
            ...
            internal override bool AddIfNotPresent(T item)
            {
                ...
                {
                    throw new ArgumentOutOfRangeException("item out of range");
                }
            ...
            internal override bool InOrderTreeWalk(TreeWalkPredicate<T> action, Boolean reverse)
            {
                ...
                // The maximum height of a red-black tree is 2*lg(n+1).
                // See page 264 of "Introduction to algorithms" by Thomas H. Cormen
                Stack<Node> stack = new Stack<Node>(2 * (int) RBTree<T>.log2(count + 1)); //this is not exactly right if count is out of date, but the stack can grow
                Node current = root;
                while (current != null)
                {
            ...
            internal override RBTree<T>.Node FindNode(T item)
            {
            ...
            public override RBTree<T> GetViewBetween(T lowerValue, T upperValue)
            {
            ...
            private void OnDeserializationImpl(Object sender)
            {
                if (siInfo == null)
                {
                    throw new SerializationException("invalid on deserialization");
                }
        ...
        public struct Enumerator : IEnumerator<T>, IEnumerator {
...
#endif
            internal Enumerator(RBTree<T> set)
            {
                tree = set;
                ...
                stack = new Stack<RBTree<T>.Node>(2 * (int) RBTree<T>.log2(set.Count + 1));
                current = null;
                reverse = false;
#if !FEATURE_NETCORE
                siInfo = null;
            ...
            internal Enumerator(RBTree<T> set, bool reverse)
            {
                tree = set;
                //this is a hack to make sure that the underlying subset has not been changed since
                //
            ...
            private void GetObjectData(SerializationInfo info, StreamingContext context)
            {
                ...
                info.AddValue(TreeName, tree, typeof(RBTree<T>));
            ...
            private void OnDeserialization(Object sender)
            {
                ...
                tree = (RBTree<T>) siInfo.GetValue(TreeName, typeof(RBTree<T>));
                ...
                stack = new Stack<RBTree<T>.Node>(2 * (int) RBTree<T>.log2(tree.Count + 1));
                current = null;
                if (EnumStarted)
                {
            ...
            public bool MoveNext()
            {
                ...
                current = stack.Pop();
                RBTree<T>.Node node = (reverse ? current.Left : current.Right);
                Node next = null, other = null;
                while (node != null)
                {
                    next = (reverse ? node.Right : node.Left);
    ...
    /// <summary>
    /// A class that generates an IEqualityComparer for this RBTree. Requires that the definition of
    /// equality defined by the IComparer for this RBTree be consistent with the default IEqualityComparer
    /// for the type T. If not, such an IEqualityComparer should be provided through the constructor.
    /// </summary>    
    internal class SortedSetEqualityComparer<T> : IEqualityComparer<RBTree<T>>
    {
        ...
        public bool Equals(RBTree<T> x, RBTree<T> y)
        {
            return RBTree<T>.SortedSetEquals(x, y, comparer);
        }
        //IMPORTANT: this part uses the fact that GetHashCode() is consistent with the notion of equality in
        //the set
        public int GetHashCode(RBTree<T> obj)
        {
            int hashCode = 0;
            if (obj != null)
            {
